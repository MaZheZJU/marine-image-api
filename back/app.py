...
def predict_with_yolo_classifier(model, image: Image.Image, device: str, model_name: str) -> Dict[str, Any]:
    ...
    print(f"[DEBUG] {model_name} raw output type={type(logits)}, shape={tuple(logits.shape) if isinstance(logits, torch.Tensor) else 'N/A'}")
...
def predict_with_yolo_detector(model, image: Image.Image, device: str, model_name: str) -> Dict[str, Any]:
    ...
    det = pred[0]  # [N, 6] => x1,y1,x2,y2,conf,cls
    class_names = model.names if hasattr(model, "names") else {}
...
def fuse_biological_results(
    ...
    """
    简单融合策略：
    1. 大类（fish/coral）优先相信 fish_coral_classifier
    2. 物种名优先用对应 YOLO 分类模型
    3. BioCLIP2 作为生物学链条/术语补充
    """
